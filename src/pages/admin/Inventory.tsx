...
export default function Inventory() {
  ...
  const checkIfCanDelete = async (id: string) => {
    try {
      ...
      console.error("Error Checking Rental Status:", e.message);
      // Proceed with deletion if rental check fails
      setDeleteConfirmId(id);
    }
  };
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Product Code", "Product Model", "Description", "Category", "Purchase Price", "Sell Price", "Stock", "Stock Value (₹)"];
    const csvContent = [
      headers.join(","),
      ...laptops.map(l => [
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setIsDialogOpen(true); setEditingLaptop(null); setSelectedFiles([]); setExistingUrls([]); setThumbnailUrl(""); reset({name:'', description: '', category: 'General', purchase_price:0, sell_price: 0, stock:0}); }} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-xs font-medium transition-colors">
              + Add Product
            </button>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input type="number" step="0.01" {...register("purchase_price", { required: true })} className="w-full rounded border border-slate-300 px-3 py-1.5 text-sm" />
                </div>
                <div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-500 border-b border-slate-100">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold text-[11px] uppercase tracking-wider min-w-[130px]">Product Code</th>
                <th className="px-4 py-3 font-semibold text-[11px] uppercase tracking-wider min-w-[320px] max-w-lg">Product Model</th>
                ...
                <th className="px-4 py-3 font-semibold text-[11px] uppercase tracking-wider">Stock Value (₹)</th>
            ...
            <tbody className="divide-y divide-slate-100">
                ...
                <tr key={l.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 min-w-[320px] max-w-lg">
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-800 text-xs whitespace-normal">{l.name}</span>
                      <span className="text-[10px] text-slate-500 whitespace-normal mt-0.5">{l.description}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-medium">{l.category || 'General'}</span>
                  </td>
                  <td className="px-4 py-3 text-slate-600 text-xs">{l.purchase_price ? `₹${l.purchase_price}` : '-'}</td>
                  ...
                  <td className="px-4 py-3 text-slate-700 text-xs font-bold">
                    {l.purchase_price && l.stock ? `₹${(l.purchase_price * l.stock).toLocaleString()}` : '-'}
                  </td>
              ...
              {laptops.length === 0 && (
                <tr>
                  <td colSpan={9} className="px-4 py-6 text-center text-xs text-slate-500">No inventory found.</td>
                </tr>
              )}
            </tbody>
          </table>
